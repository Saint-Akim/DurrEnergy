...
def aggregate_system_performance(df, system_label):
    ...
    Technical Method:
    1. For new system: Sum individual inverter outputs per timestamp
    2. For old system: Use calculated instantaneous power
    3. Compute hourly, daily, and statistical metrics
    4. Apply solar-appropriate filtering (daylight hours only)
    """
    if df.empty:
        return pd.DataFrame()
    ...
    try:
        ...
        # Method: Average power × effective daylight hours
...
def calculate_performance_improvements(old_metrics, new_metrics):
    ...
    Returns statistical summary with confidence levels
    """
...
def calculate_seasonal_factor(sample_date):
    ...
    Technical basis: Solar irradiance varies ~30% between winter/summer
    """
